...
class ContinuumRobotController:
    ...
    def muscle_lengths_root_solver(self, L, k, phi, d, s):
        ...
        f1 = (L1 + L2 + L3) / 3 - s
        f2 = (np.sqrt(3) * (L2 + L3 - 2 * L1)) / (3 * (L2 - L3)) - np.tan(phi)
        f3 = num / den - k

        return [f1, f2, f3]

    def MuscleLengths(self, curvature, phi, BackboneLength, VertebraDiam):
        # Finds the change in the muscles lengths from the resting Length using root solving least squares.

        x0 = BackboneLength + np.random.rand(3)  # initial guess
        ...
        if result.success:
            return (result.x ) # Calculate the change in length.
        else:
    ...
    def PressuresFromMoment(self, M, backboneLength, curvature, phi, VertebraDiam, MuscleStiffness, ForcePressureSensitivity, MuscleRestLength):
        ...
        deltaL= self.MuscleLengths(curvature, phi, backboneLength, VertebraDiam)- MuscleRestLength

        # each column is the coordinates d = -[d_y, d_x] of the muscle's start relative to the origin
        J = np.array([[-1, -np.sin(-math.pi/6), -np.sin(-5*math.pi/6)],
                      [0, -np.cos(-math.pi/6), -np.cos(-5*math.pi/6)]])
        ...
        P = lsq_linear(J, (np.transpose(M)-NumMuscles*J*(MuscleStiffness*deltaL))/ForcePressureSensitivity, bounds=(0, self.maxP))
    ...
    def momentFromCurvature(self,curvature, phi, YoungsModulus, CoreDiameter):
        ...
        M = [curvature*YoungsModulus*I*np.cos(phi), curvature*YoungsModulus*I*np.sin(phi)]
    ...
    def ControllerThread(self): # Controller thread--> this is where you should put your MPC controller
        ...
        ForcePressureSensitivity =.07 #N/psi
        MuscleRestLength=1 #m
        VertebraDiam=1 #m
        ...
        while self._running:
            ...
            u = pid.compute([phi, curvature], [phi_desired, curvature_desired], dt)

            # Use constant curvature and muscle model to find P with FF controller
            P = self.PressuresFromMoment(u+[phi_desired, curvature_desired], BackboneLength, curvature_desired, phi_desired,
